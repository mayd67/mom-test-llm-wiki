...
**基本信息**
- 文档版本：v1.4 | 更新日期：2025-12-29 | 维护团队：产品研发团队
- 目标受众：产品研发团队
...
| 五、质量检验 | 如何管理检验任务、检验报工和不合格品审理？ | 定义检验分类配置、检验任务、检验报工凭证、质量填报模版和不合格品处理相关模型 |
...
**模型类型:** 业务模型 | **父模型:** `BusinessObject` | **接口:** `FactoryManaged`、`ExternalDataManaged`、`LifecycleManaged`、`SecurityManaged` | **表名:** `MOM_PROD_ORDER`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 名称 | `name` | `CNAME` | 字符串 | 512 | 无 | |
| 订单类型 | `orderType` | `CORDER_TYPE` | 枚举 | 256 | 无 | 引用枚举:OrderType |
| 物料 | `material` | `CMATERIAL` | 引用对象 | - | 必填 | 引用Material |
| 物料清单 | `bom` | `CBOM` | 引用对象 | - | 无 | 引用BOM |
| 制造型号 | `manuModel` | `CMANU_MODEL` | 字符串 | 256 | 无 | |
| 工艺路线 | `routing` | `CROUTING` | 引用对象 | - | 无 | 引用Routing |
| 计量单位 | `unit` | `CUNIT` | 单位 | 256 | 必填 | 数量类型单位 |
| 计划数量 | `plannedQty` | `CPLANNED_QTY` | 浮点型 | - | 必填 | |
| 计划产出数量 | `plannedOutputQty` | `CPLANNED_OUTPUT_QTY` | 浮点型 | - | 无 | |
...
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 初始(PO_010_CREATED) \|已展开(PO_030_EXPANDED)\|已释放(PO_040_RELEASED)\|已开工(PO_050_STARTED) \|已完工(PO_060_FINISHED) |
| 释放状态 | `releasedStatus` | `CRELEASED_STATUS` | 枚举 | 256 | 无 | 枚举值：未释放(PO_010_NO) \| 部分释放(PO_020_PART) \| 全部释放(PO_030_ALL) |
| 计划类型 | `prodPlanType` | `CPROD_PLAN_TYPE` | 枚举 | 256 | 无 | 枚举值：无(PPT_010_NONE) \| 零部件生产计划(PPT_030_PRODUCE) \| 零部件加工计划(PPT_050_PROCESS) |
| 排产状态 | `prodScheduleState` | `CPROD_SCHEDULE_STATE` | 枚举 | 256 | 无 | 枚举值：未排产(PSS_010_NONE) \| 零部件生产已排产(PSS_030_PRODUCE) |

### 2.2 生产订单一级工艺展开结构表（ProdOrderPrExpandLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_PROD_ORDER_PR_EXPAND_LINK`
...
| 子订单所属工厂 | `childOrderFactory` | `CCHILD_ORDER_FACTORY` | 引用对象 | - | 无 | 引用Org |
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `FactoryManaged` | **表名:** `MOM_PROD_ORDER_RELEASE_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_PROD_ORDER_PREPARATION_LINK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 物料 | `material` | `CMATERIAL` | 引用对象 | - | 必填 | |
| 工序编号 | `processCode` | `CPROCESS_CODE` | 字符串 | 255 | 无 | |
| 工序名称 | `processName` | `CPROCESS_NAME` | 字符串 | 255 | 无 | |
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_PROD_ORDER_ACTION_RECORD_LINK`
...
| 操作内容 | `actionContent` | `CACTION_CONTENT` | 文本大对象 | - | 无 | |
...
**模型类型:** 业务模型 | **父模型:** `BusinessObject` | **接口:** `FactoryManaged`、`ExternalDataManaged`、`LifecycleManaged`、`SecurityManaged` | **表名:** `MOM_MANU_ORDER`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 订单制造类型 | `manuType` | `CMANU_TYPE` | 枚举 | 256 | 必填 | 引用枚举:ManuType |
| 生产订单 | `prodOrder` | `CPROD_ORDER` | 引用对象 | - | 必填 | 引用ProdOrder |
| 物料 | `material` | `CMATERIAL` | 引用对象 | - | 必填 | 引用Material |
| 制造型号 | `manuModel` | `CMANU_MODEL` | 字符串 | 256 | 无 | |
| 物料清单 | `bom` | `CBOM` | 引用对象 | - | 无 | 引用BOM |
| 工艺路线 | `routing` | `CROUTING` | 引用对象 | - | 无 | 引用Routing |
| 计量单位 | `unit` | `CUNIT` | 单位 | 256 | 必填 | 数量计量单位 |
| 计划数量 | `plannedQty` | `CPLANNED_QTY` | 浮点型 | - | 必填 | |
...
| 指定计划结束时间 | `specifiedPlannedEndTime` | `CSPECIFIED_PLANNED_END_TIME` | 日期时间 | - | 无 | 对应交货期 |
| 优先级 | `priority` | `CPRIORITY` | 整型 | - | 无 | |
| 计划员 | `planner` | `CPLANNER` | 长整型 | - | 无 | |
| 控制状态 | `controlStatus` | `CCONTROL_STATUS` | 枚举 | 256 | 必填 | 枚举值：正常(CS_010_NORMAL) \| 暂停(CS_020_PAUSED) \| 取消(CS_030_CANCEL) |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 初始(MO_010_CREATED)\|   已展开(MO_030_EXPANDED) \|已开工(MO_050_STARTED)\| 已完工(MO_060_FINISHED) |
...
| 源制造订单 | sourceManuOrder | CSOURCE_MANU_ORDER | 引用对象 |  |  |  |
| 是否入库 | storedFlag | CSTORED_FLAG | 布尔 | | | 默认false |

### 2.7 制造订单与不合格品审理单结论关系（ManuOrderDefectiveReviewRelation）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MANU_ORDER_DEFECTIVE_REVIEW_RELATION`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 不合格品审理结论 | `defectiveReviewConclusion` | `CDEFECTIVE_REVIEW_CONCLUSION` | 引用对象 | - | 无 | |
| 不合格品子件审理结论 | `defectiveReviewSubConclusion` | `CDEFECTIVE_REVIEW_SUB_CONCLUSION` | 引用对象 | - | 无 | |
| 审理结论类型 | `conclusionType` | `CCONCLUSION_TYPE` | 枚举 | - | 无 | 引用枚举:ConclusionType |

### 2.8 制造订单动作变迁记录（ManuOrderActionRecordLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MANU_ORDER_ACTION_RECORD_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MANU_ORDER_SERIAL_NUMBER_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MANU_ORDER_OUT_PUT_SERIAL_NUMBER_LINK`
...
**模型类型:** 业务模型 | **父模型:** `BusinessObject` | **接口:** `FactoryManaged`、`ExternalDataManaged`、`SecurityManaged` | **表名:** `MOM_MANU_TASK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 生产订单 | `prodOrder` | `CPROD_ORDER` | 引用对象 | - | 无 | 引用ProdOrder |
| 生产订单号 | `prodOrderCode` | `CPROD_ORDER_CODE` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 制造订单 | `manuOrder` | `CMANU_ORDER` | 引用对象 | - | 无 | 引用ManuOrder |
| 制造订单号 | `manuOrderCode` | `CMANU_ORDER_CODE` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 物料 | `material` | `CMATERIAL` | 引用对象 | - | 必填 | 引用Material |
| 物料名称 | `materialName` | `CMATERIAL_NAME` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 物料编码 | `materialCode` | `CMATERIAL_CODE` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 物料图号 | `materialDwgNumber` | `CMATERIAL_DWG_NUMBER` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 物料版本 | `materialVersion` | `CMATERIAL_VERSION` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 工艺路线 | `routing` | `CROUTING` | 引用对象 | - | 必填 | 引用Routing |
| 工艺路线名称 | `routingName` | `CROUTING_NAME` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 工艺路线编码 | `routingCode` | `CROUTING_CODE` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 工艺路线版本 | `routingVersion` | `CROUTING_VERSION` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 计量单位 | `unit` | `CUNIT` | 单位 | 256 | 必填 | |
| 计划数量 | `plannedQty` | `CPLANNED_QTY` | 浮点型 | - | 必填 | |
| 合格数量 | `qualifiedQty` | `CQUALIFIED_QTY` | 浮点型 | - | 无 | 冗余回写字段 |
| 报废数量 | `scrappedQty` | `CSCRAPPED_QTY` | 浮点型 | - | 无 | 冗余回写字段 |
...
| 工序名称 | `processName` | `CPROCESS_NAME` | 字符串 | - | 无 | 冗余存储（工控网隔离） |
| 工序类型 | `processType` | `CPROCESS_TYPE` | 枚举 | - | 无 | 引用枚举:ProcessType |
| 工作中心 | `workCenter` | `CWORK_CENTER` | 引用对象 | - | 无 | 引用WorkCenter |
| 设备 | `equipment` | `CEQUIPMENT` | 引用对象 | - | 无 | 引用Equipment |
| 计划开始时间 | `plannedStartTime` | `CPLANNED_START_TIME` | 日期时间 | - | 无 | |
| 计划结束时间 | `plannedEndTime` | `CPLANNED_END_TIME` | 日期时间 | - | 必填 | |
| 实际开始时间 | `actualStartTime` | `CACTUAL_START_TIME` | 日期时间 | - | 无 | |
| 实际结束时间 | `actualEndTime` | `CACTUAL_END_TIME` | 日期时间 | - | 无 | |
| 指定工作中心 | `specifiedWorkCenter` | `CSPECIFIED_WORK_CENTER` | 引用对象 | - | 无 | 适用于排产锁定工作中心 |
| 控制状态 | `controlStatus` | `CCONTROL_STATUS` | 枚举 | 256 | 必填 | 引用枚举:ControlStatus |
...
| 首检状态 | `firstInspectStatus` | `CFIRST_INSPECT_STATUS` | 枚举 | - | 无 | 枚举值：初始(CREATED) \| 合格(QUALIFIED) \| 报废(SCRAPPED) |
| 派工方式 | `dispatchMode` | `CDISPATCH_MODE` | 枚举 | - | 无 | 枚举值：未派工(DM_10_NONE) \| 自动派工(DM_20_AUTO) \| 手动派工(DM_30_MANUAL)，默认值：DM_10_NONE |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 初始(MT_010_CREATED) \|已派工(MT_020_DISPATCHED) \| 已确认(MT_030_CONFIRMED) \|已开工(MT_040_STARTED) 已送检(MT_050_INSPECTED)  已完工(MT_060_FINISHED) |

### 2.13 制造任务动作变迁记录（ManuTaskActionRecordLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MANU_TASK_ACTION_RECORD_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MANU_TASK_PREV_RELATION_LINK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 当前任务 | `taskId` | `CTASK_ID` | 引用对象 | - | 必填 | |
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MANU_TASK_EXECUTOR_LINK`
...
| 制造订单 | `manuOrder` | `CMANU_ORDER` | 引用对象 | - | 必填 | 引用ManuOrder |
| 控制状态 | `controlStatus` | `CCONTROL_STATUS` | 枚举 | 256 | 必填 | 枚举值：正常(CS_010_NORMAL) \| 暂停(CS_020_PAUSED) \| 取消(CS_030_CANCEL) |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 初始(MPP_010_CREATED) \|  备料中(MPP_020_PREPARATION) \|  备料完成(MPP_030_COMPLETED) |

### 3.2 物料准备计划明细（MaterialPreparationPlanDetailLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **表名:** `MOM_MATERIAL_PREPARE_PLAN_DETAIL_LINK`
...
| 需求数量 | `requireQty` | `CREQUIRE_QTY` | 浮点型 | - | 必填 | |
| 子件比例 | `componentRatio` | `CCOMPONENT_RATIO` | 整型 | - | 必填 | 默认为1 |
| 已申请数量 | `requestQty` | `CREQUIR_QTY` | 浮点型 | - | 必填 | 默认为0 |
...
| 替换件物料 | `replacementMaterial` | `CREPLACEMENT_MATERIAL` | 引用对象 | - | 无 | 引用Material |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 初始(MPP_010_CREATED) \|已申请(MPP_020_PREPARATION)\|已收料(MPP_030_COMPLETED) |

### 3.3 领料申请明细（MaterialPreparationPlanRequestDetailLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MATERIAL_PREPARATION_PLAN_REQUEST_DETAIL_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MATERIAL_PREPARATION_PLAN_RECEIVING_DETAIL_LINK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 出库单 | `outStoreRecord` | `COUT_STORE_RECORD` | 引用对象 | - | 必填 | |
| 物料准备计划明细 | `materialPreparationPlanDetail` | `CMATERIAL_PREPARE_PLAN_DETAIL` | 引用对象 | - | 必填 | |
| 实际装入物料 | `receivedMaterial` | `CRECEIVED_MATERIAL` | 引用对象 | - | 无 | 引用Material |
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MATERIAL_PREPARATION_PLAN_RETURN_DETAIL_LINK`
...
| 申请人 | `applicant` | `CAPPLICANT` | 用户 | - | 必填 | |
| 申请时间 | `applicantTime` | `CAPPLICANT_TIME` | 日期时间 | - | 必填 | |
| 退料原因 | `returnReason` | `CRETURN_REASON` | 字符串 | - | 必填 | |
| 退料数量 | returnQty | CRETURN_QTY | 浮点 | 不涉及 | 必填 | |

### 3.6 物料装入清单（MaterialLoadList）

**模型类型:** 业务模型 | **父模型:** `BusinessObject` | **接口:** 无 | **表名:** `MOM_MATERIAL_LOAD_LIST`
...
| 工序编号 | `processCode` | `CPROCESS_CODE` | 字符串 | 255 | 无 | |
| 工序名称 | `processName` | `CPROCESS_NAME` | 字符串 | 255 | 无 | |
| 批次号 | `batchNumber` | `CBATCH_NUMBER` | 字符串 | 255 | 无 | |
| 序列号 | `serialNumber` | `CSERIAL_NUMBER` | 字符串 | 255 | 无 | |
| 需求数量 | `requireQty` | `CREQUIRE_QTY` | 浮点型 | - | 必填 | |
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MATERIAL_ACTUAL_LOAD_LINK`
...
### 3.8 物料装入操作记录（MaterialLoadRecordLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_MATERIAL_LOAD_RECORD_LINK`
...
| 批次号 | `batchNumber` | `CBATCH_NUMBER` | 字符串 | 255 | 无 | |
| 序列号 | `serialNumber` | `CSERIAL_NUMBER` | 字符串 | 255 | 无 | |
| 操作类型 | `loadType` | `CLOAD_TYPE` | 枚举 | 50 | 必填 | 引用枚举:LoadType |
| 操作人 | `operator` | `COPERATOR` | 用户 | - | 无 | |
| 操作数量 | `operatorQty` | `COPERATOR_QTY` | 浮点型 | - | 必填 | |
| 操作时间 | `operatorTime` | `COPERATOR_TIME` | 日期时间 | - | 必填 | |
| 操作说明 | `operatorRemark` | `COPERATOR_REMARK` | 字符串 | - | 无 | |

### 3.9 在制品（Wip）

**模型类型:** 业务模型 | **父模型:** `BusinessObject` | **接口:** `FactoryManaged`、`ExternalDataManaged`、`SecurityManaged` | **表名:** `MOM_WIP`
...
| 制造任务 | `manuTask` | `CMANU_TASK` | 引用对象 | - | 无 | 引用ManuTask |
| 数量 | `qty` | `CQTY` | 浮点型 | - | 无 | |
| 状态 | `wipStatus` | `CWIP_STATUS` | 枚举 | 256 | 无 | 引用枚举:WipStatus |
| 下道任务 | `nextManuTask` | `CNEXT_MANU_TASK` | 引用对象 | - | 无 | 引用ManuTask |

### 3.10 报工凭证（ReportProof）

**模型类型:** 业务模型 | **父模型:** `BaseObject` | **接口:** `FactoryManaged`、`ExternalDataManaged`、`SecurityManaged` | **表名:** `MOM_REPORT_PROOF`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `SecurityManaged` | **表名:** `MOM_REPORT_PROOF_ITEM_LINK`
...
| 汇报项类型 | `itemType` | `CITEM_TYPE` | 字符串 | 256 | 必填 | |
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `SecurityManaged` | **表名:** `MOM_REPORT_PROOF_EXECUTOR_LINK`
...
**模型类型:** 业务模型 | **父模型:** `BaseObject` | **接口:** 无 | **表名:** `MOM_MANU_ORDER_SPLIT_RELATION_LINK`
...
| 分卡工序号 | `splitProcCode` | `CSPLIT_PROC_CODE` | 字符串 | - | 必填 | |
| 源制造订单根节点 | `rootSourceManuOrder` | `CROOT_SOURCE_MANU_ORDER` | 引用对象 | - | 必填 | |
| 源制造订单全路径 | `fullPathSourceManuOrder` | `CFULL_PATH_SOURCE_MANU_ORDER` | 字符串 | - | 必填 | |
| 源制造任务根节点 | `rootSourceManuTask` | `CROOT_SOURCE_MANU_TASK` | 引用对象 | - | 必填 | |
| 源制造任务全路径 | `fullPathSourceManuTask` | `CFULL_PATH_SOURCE_MANU_TASK` | 字符串 | - | 必填 | |
| 汇报项编码 | `itemCode` | `CITEM_CODE` | 字符串 | - | 无 | |
| 汇报项类型 | `itemType` | `CITEM_TYPE` | 字符串 | - | 无 | |
| 汇报项名称 | `itemName` | `CITEM_NAME` | 字符串 | - | 必填 | |
...
**模型类型:** 业务模型 | **父模型:** `BusinessObject` | **接口:** `FactoryManaged`、`ExternalDataManaged` | **表名:** `MOM_INSPECT_TASK`
...
| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明                                                                                                                                                   |
|-------------|-------------|-----------|---------|------|------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| 业务状态 | `bizStatus` | `CBIZ_STATUS` | 枚举 | - | 无 | 初始:IT_100_CREATED\|检验中:IT_200_PROCESS\|已完工:IT_300_FINISHED                                                                                           |
| 生产订单 | `prodOrder` | `CPROD_ORDER` | 引用对象 | - | 无 | 引用ProdOrder                                                                                                                                          |
...
| 检验分类 | `inspectCategory` | `CINSPECT_CATEGORY` | 枚举 | - | 必填 | 枚举值：自检(INS_CATEGORY_100_SELF)\|首检(INS_CATEGORY_150_FIRST)\|互检(INS_CATEGORY_200_MUTUAL)\|专检(INS_CATEGORY_300_SPECIAL)\|客户检(INS_CATEGORY_400_CUSTOMER) |
...
| 实际开始时间 | `actualStartTime` | `CACTUAL_START_TIME` | 日期时间 | - | 无 |                                                                                                                                                      |
| 实际结束时间 | `actualEndTime` | `CACTUAL_END_TIME` | 日期时间 | - | 无 |                                                                                                                                                      |
| 控制状态 | `controlStatus` | `CCONTROL_STATUS` | 枚举 | - | 必填 | 枚举值：正常(CS_010_NORMAL)\|暂停(CS_020_PAUSED)                                                                                                             |
| 启用序列号标记 | `serialNumberFlag` | `CSERIAL_NUMBER_FLAG` | 布尔 | - | 无 |                                                                                                                                                      |
| 执行标记 | `executionFlag` | `CEXECUTION_FLAG` | 布尔 | - | 无 |                                                                                                                                                      |
| 批次号 | `batchNumber` | `CBATCH_NUMBER` | 字符串 | 256 | 无 |                                                                                                                                                      |
...
| 报废数量 | `scrappedQty` | `CSCRAPPED_QTY` | 浮点型 | - | 无 | 冗余回写字段                                                                                                                                               |
| 待定数量 | `pendingQty` | `CPENDING_QTY` | 浮点型 | - | 无 | 冗余回写字段                                                                                                                                               |
| 不合格审批状态 | `defectiveApprovalStatus` | `CDEFECTIVE_APPROVAL_STATUS` | 枚举 | - | 必填 | 默认值DF_010_NONE，枚举值：无审批(DF_010_NONE)\|审批中(DF_020_PROCESSING)\|审批完成(DF_030_FINISHED)                                                                   |

### 5.3 检验任务接收序列号（InspectTaskReceivedSnLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `FactoryManaged` | **表名:** `MOM_INSPECT_TASK_RECEIVED_SN_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_INSPECT_ACTION_RECORD_LINK`
...
**模型类型:** 业务模型 | **父模型:** `BaseObject` | **接口:** `FactoryManaged`、`ExternalDataManaged`、`SecurityManaged` | **表名:** `MOM_INSPECT_REPORT_PROOF`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `SecurityManaged`、`FactoryManaged` | **表名:** `MOM_INSPECT_REPORT_PROOF_ITEM_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `SecurityManaged`、`FactoryManaged` | **表名:** `MOM_INSPECT_REPORT_PROOF_EXECUTOR_LINK`
...
| 设计内容 | `designContent` | `CDESIGN_CONTENT` | 文本大对象 | - | 无 | 存储设计json |

### 5.9 质量填报模版关联（QMRecordTmplBindLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `FactoryManaged` | **表名:** `MOM_QM_RECORD_TMPL_BIND_LINK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 序号 | `index` | `CINDEX` | 整型 | - | 必填 | |
| 绑定数据模型 | `bindEntityType` | `CBIND_ENTITY_TYPE` | 枚举 | 128 | 必填 | 枚举值：物料(MATERIAL)\|工艺(ROUTING)\|工序(PROC) |
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `QMRecordManaged` | **表名:** `MOM_QM_SMELT_REPORT_OPT_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** `QMRecordManaged` | **表名:** `MOM_QM_SMELT_REPORT_RECORD_LINK`
...
| 数量 | `qty` | `CQTY` | 浮点型 | - | 无 | |
| 物料 | `material` | `CMATERIAL` | 引用对象 | - | 无 | 引用Material |
| 批次号 | `batchNumber` | `CBATCH_NUMBER` | 字符串 | - | 无 | |

### 5.12 不合格品主件审理结论（DefectiveProductReviewConclusionLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_DEFECTIVE_PRODUCT_REVIEW_CONCLUSION_LINK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 顺序号 | `serialNumber` | `CSERIAL_NUMBER` | 字符串 | - | 无 | |
| 数量 | `qty` | `CQTY` | 浮点型 | - | 无 | |
| 结论 | `reviewConclusion` | `CREVIEW_CONCLUSION` | 枚举 | - | 无 | 引用枚举:ReviewConclusion |
| 分卡标记 | `splitFlag` | `CSPLIT_FLAG` | 布尔 | - | 无 | 是否分卡 |
| 状态 | `status` | `CSTATUS` | 枚举 | - | 无 | 待处理(PENDING) \| 已完成(COMPLETED) |
| 备注 | `remark` | `CREMARK` | 字符串 | - | 无 | |

### 5.13 不合格品子件审理结论（DefectiveReviewSubConclusionLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_DEFECTIVE_REVIEW_SUB_CONCLUSION_LINK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 主件序列号 | `mainPartSerialNumber` | `CMAIN_PART_SERIAL_NUMBER` | 字符串 | - | 无 | |
...
| 来源类型 | `sourceType` | `CSOURCE_TYPE` | 枚举 | - | 必填 | 引用枚举:SourceType |
| 是否连续外委 | `continuousFlag` | `CCONTINUOUS_FLAG` | 布尔 | - | 无 | |
| 不合格品子件审理结论 | `defectiveReviewSubConclusion` | `CDEFECTIVE_REVIEW_SUB_CONCLUSION` | 引用对象 | - | 无 | |
| 业务状态             | bizStatus                      | CBIZ_STATUS                        | 枚举     | 256  | 必填 | 初始(OR_10_INITIAL)\|已发送 (MO_20_PUBLISHED) \| 已完成 (MO_30_COMPLETED) |

**生命周期状态：** 已创建(OR_10_INITIAL) ||已发送(OR_40_PUBLISHED) | 已完成(OR_60_COMPLETED)

### 6.2 外委需求明细（OutsourcingRequireDetailLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_OUTSOURCING_REQUIRE_DETAIL_LINK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 制造任务 | `manuTask` | `CMANU_TASK` | 引用对象 | - | 必填 | 引用ManuTask |
| 数量 | `qty` | `CQTY` | 浮点型 | - | 无 | |
| 是否首道 | `isFirst` | `CIS_FIRST` | 布尔 | - | 无 | 记录连续工序的头工序 |
| 是否末道 | `isLast` | `CIS_LAST` | 布尔 | - | 无 | 记录连续工序的尾工序 |
...
| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| PO号 | `poNumber` | `CPO_NUMBER` | 字符串 | - | 无 | erp来源 |
| 计划数量 | `plannedQty` | `CPLANNED_QTY` | 浮点型 | - | 必填 | |
...
| 供应商 | `supplier` | `CSUPPLIER` | 引用对象 | - | 无 | |
| 物料 | `material` | `CMATERIAL` | 引用对象 | - | 无 | 引用Material |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 初始(OPO_10_INITIAL)\|已发货(OPO_20_SHIPPED) \|部分收货(OPO_30_PARTIAL_RECEIVED)  \|全部收货(OPO_40_FULLY_RECEIVED)  \|已入库(OPO_50_IN_STOCK) \|已退货(OPO_60_RETURNED) |
...
| 发货时间 | `deliveryTime` | `CDELIVERY_TIME` | 日期时间 | - | 无 | |
| 物流信息 | `logisticsInfo` | `CLOGISTICS_INFO` | 字符串 | - | 无 | |
...
| 退货数量 | `returnQty` | `CRETURN_QTY` | 浮点型 | - | 无 | |
| 收货时间 | `receiptTime` | `CRECEIPT_TIME` | 日期时间 | - | 无 | |
| 收货类型 | `receiptType` | `CRECEIPT_TYPE` | 枚举 | - | 无 | 引用枚举:ReceiptType |
| 采购订单 | `purchaseOrder` | `CPURCHASE_ORDER` | 引用对象 | - | 无 | |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 待检验(ORR_10_PENDING_INSPECTION)  \| 已完成(ORR_20_COMPLETED) |

### 6.6 外委收货单明细（OutsourcingReceiptOrderLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_OUTSOURCING_RECEIPT_ORDER_LINK`
...
**模型类型:** 业务模型 | **父模型:** `BaseObject` | **接口:** `FactoryManaged`、`SecurityManaged`、 | **表名:** `MOM_OUTSOURCING_RETURN_ORDER`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 退货单类型 | `returnType` | `CRETURN_TYPE` | 枚举 | - | 无 | 引用枚举:ReturnType |
| 物料 | `material` | `CMATERIAL` | 引用对象 | - | 必填 | 引用Material |
| 退货数量 | `returnQty` | `CRETURN_QTY` | 浮点型 | - | 必填 | |
| 退货原因 | `returnReason` | `CRETURN_REASON` | 字符串 | - | 必填 | |
| 收货单 | `receiptOrder` | `CRECEIPT_ORDER` | 引用对象 | - | 无 | |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 待退货(ORO_10_PENDING_RETURN) \| 已退货(ORO_20_RETURNED) |
...
| 优先级 | `priorityLevel` | `CPRIORITY_LEVEL` | 枚举 | - | 无 | 引用枚举:PriorityLevel |
| 物料 | `material` | `CMATERIAL` | 引用对象 | - | 必填 | 引用Material |
| 数量 | `qty` | `CQTY` | 浮点型 | - | 必填 | |
| 期望到货时间 | `expectedArrivalTime` | `CEXPECTED_ARRIVAL_TIME` | 日期时间 | - | 必填 | |
| 供应商 | `supplier` | `CSUPPLIER` | 引用对象 | - | 无 | |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 已创建(PR_10_INITIAL) \|  已发送(PR_20_PUBLISHED) |
...
| 计划数量 | `plannedQty` | `CPLANNED_QTY` | 浮点型 | - | 必填 | |
| 已收货数量 | `receivedQty` | `CRECEIVED_QTY` | 浮点型 | - | 无 | |
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 初始(PO_10_INITIAL) \| 部分收货(PO_30_PARTIAL_RECEIVED) \| 全部收货(PO_40_FULLY_RECEIVED)  \|已入库(PO_50_IN_STOCK) \|已退货(PO_60_RETURNED) |
...
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 待检验(RR_10_PENDING_INSPECTION) \|  已完成(RR_20_COMPLETED) |

### 7.4 采购收货单明细（PurchaseReceiptLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_PURCHASE_RECEIPT_LINK`
...
| 业务状态 | bizStatus | CBIZ_STATUS | 枚举 | 256 | 必填 | 待退货(RO_10_PENDING_RETURN) \| 已退货(RO_20_RETURNED) |
...
| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 流程标识 | `flowCode` | `CFLOW_CODE` | 字符串 | 255 | 无 | |
| 是否预定义 | `predefinedFlag` | `CPREDEFINED_FLAG` | 布尔 | - | 必填 | 默认值：false |

### 8.2 异常描述库（AbnormalDescription）

**模型类型:** 业务模型 | **父模型:** `BaseObject` | **接口:** 无 | **表名:** `MOM_ABNORMAL_DESCRIPTION`
...
| 设备 | `equip` | `CEQUIP` | 引用对象 | - | 无 | |
| 承诺解决时间 | `promiseResolutionTime` | `CPROMISE_RESOLUTION_TIME` | 日期时间 | - | 无 | |
| 关闭人 | `closer` | `CCLOSER` | 用户 | - | 无 | |
| 关闭时间 | `closeTime` | `CCLOSE_TIME` | 日期时间 | - | 无 | |
| 发起人 | `initiator` | `CINITIATOR` | 用户 | - | 无 | |
| 是否加入审批流 | `joinFlowFlag` | `CJOIN_FLOW_FLAG` | 布尔 | - | 必填 | 默认值：false |
| 关闭说明 | `closureExplanation` | `CCLOSURE_EXPLANATION` | 文本大对象 | - | 无 | |

### 8.4 异常操作记录（AbnormalTaskLogLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_ABNORMAL_TASK_LOG_LINK`

| 属性中文名称 | 属性英文名称 | 数据库列名 | 数据类型 | 长度 | 约束 | 说明 |
|-------------|-------------|-----------|---------|------|------|------|
| 操作类型 | `operationType` | `COPERATION_TYPE` | 枚举 | - | 必填 | 枚举值：创建 \| 处理 \| 关闭 |
| 操作描述 | `operationDesc` | `COPERATION_DESC` | 字符串 | - | 必填 | |
| 操作人 | `operator` | `COPERATOR` | 用户 | - | 必填 | |
| 操作时间 | `operationTime` | `COPERATION_TIME` | 日期时间 | - | 必填 | |

### 8.5 生产异常任务与附件关系（AbnormalTaskFileLink）

**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_ABNORMAL_TASK_FILE_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_ABNORMAL_TASK_MANU_TASK_LINK`
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **接口:** 无 | **表名:** `MOM_ABNORMAL_TASK_INSPECT_TASK_LINK`
