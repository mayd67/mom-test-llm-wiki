...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **表名:** `MOM_QUALITY_HISTORY_PROC_RECORD_LINK`
...
| 任务实体类型 | `taskEntityType` | `CTASK_ENTITY_TYPE` | 字符串 | 36 | 无 | 任务实体类型标识 |
...
**模型类型:** 业务模型 | **父模型:** `GenericLink` | **表名:** `MOM_QUALITY_HISTORY_PHYSICAL_BOM_LINK`
