...
| `templates/` | 仅在 skill 需要复用独立模板文件时提供，例如 Excel 导入模板 |
...
| `requirement-to-testcases` | 从需求材料或测试点生成模板化测试用例 | `references/testcase-rules.md`、`assets/testcase-template.md` | Markdown 工作稿默认保留 `用例编码` 列并进 `outputs/03_测试用例/测试用例/`，导入版 Excel 不保留该列并进 `outputs/03_测试用例/MeterSphere导入/` |
...
| `jira-defect-importer` | 将规范化缺陷 CSV 或已整理缺陷数据直接导入 Jira | `references/jira-rest-mapping.md`、`assets/jira-import-config.example.json`、`scripts/import_jira_defects.py` | 默认先做 dry-run；仅在用户明确要求时真实写入 Jira |
