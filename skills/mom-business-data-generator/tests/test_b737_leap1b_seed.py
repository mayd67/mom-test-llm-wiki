...
import build_b737_leap1b_seed as b737_seed


class B737Leap1BSeedTests(unittest.TestCase):
    ...
    def test_build_variants_cover_standard_big_and_ultra_profiles(self):
        standard = b737_seed.build_variant(namespace='B7L1B01', volume_profile='标准版')
        big = b737_seed.build_variant(namespace='B7L1B31', volume_profile='大体量版')
        ultra = b737_seed.build_variant(namespace='B7L1B42', volume_profile='超大体量版')

        self.assertEqual(standard.get('metadata', {}).get('namespace'), 'B7L1B01')
        self.assertEqual(standard.get('metadata', {}).get('volume_profile'), '标准版')
        self.assertEqual(big.get('metadata', {}).get('namespace'), 'B7L1B31')
        self.assertEqual(big.get('metadata', {}).get('volume_profile'), '大体量版')
        self.assertEqual(ultra.get('metadata', {}).get('namespace'), 'B7L1B42')
    ...
    def test_build_keeps_standard_variant_as_default(self):
        ...
        self.assertEqual(metadata.get('namespace'), 'B7L1B01')
        self.assertEqual(metadata.get('volume_profile'), '标准版')
        self.assertGreaterEqual(len(workbooks['系统配置_模板.xlsx']['用户']), 180)
        self.assertGreaterEqual(len(workbooks['工厂资源_模板.xlsx']['设备']), 20)
        self.assertGreaterEqual(len(workcenter_rows), 20)
        self.assertGreaterEqual(len(workbooks['产品与工艺_模板.xlsx']['物料']), 120)
