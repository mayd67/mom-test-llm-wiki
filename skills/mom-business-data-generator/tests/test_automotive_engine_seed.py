...
import build_automotive_engine_seed as automotive_seed


class AutomotiveEngineSeedTests(unittest.TestCase):
    ...
    def test_build_variants_cover_standard_big_and_ultra_profiles(self):
        standard = automotive_seed.build_variant(namespace='AE20T01', volume_profile='标准版')
        big = automotive_seed.build_variant(namespace='AE31T01', volume_profile='大体量版')
        ultra = automotive_seed.build_variant(namespace='AE42T01', volume_profile='超大体量版')

        self.assertEqual(standard.get('metadata', {}).get('namespace'), 'AE20T01')
        self.assertEqual(standard.get('metadata', {}).get('volume_profile'), '标准版')
        self.assertEqual(big.get('metadata', {}).get('namespace'), 'AE31T01')
        self.assertEqual(big.get('metadata', {}).get('volume_profile'), '大体量版')
        self.assertEqual(ultra.get('metadata', {}).get('namespace'), 'AE42T01')
    ...
    def test_build_keeps_standard_variant_as_default(self):
        ...
        self.assertEqual(metadata.get('namespace'), 'AE20T01')
        self.assertEqual(metadata.get('volume_profile'), '标准版')
        self.assertGreaterEqual(len(workbooks['系统配置_模板.xlsx']['用户']), 150)
        self.assertGreaterEqual(len(workbooks['工厂资源_模板.xlsx']['设备']), 30)
        self.assertGreaterEqual(len(workcenter_rows), 30)
        self.assertGreaterEqual(len(workbooks['产品与工艺_模板.xlsx']['物料']), 100)
        self.assertGreaterEqual(len(workbooks['产品与工艺_模板.xlsx']['工艺路线工序']), 150)
        self.assertGreaterEqual(len(workbooks['产品与工艺_模板.xlsx']['工艺路线工步']), 300)
        self.assertTrue(supplier_link_rows)
