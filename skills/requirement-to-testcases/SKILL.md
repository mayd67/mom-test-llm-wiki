...
- 如果输出的是 Markdown 工作稿，默认补齐字段：`用例编码`、`用例名称`、`所属模块`、`标签`、`前置条件`、`步骤描述`、`预期结果`、`编辑模式`、`备注`、`用例状态`、`责任人`、`用例等级`。
- 如果输出的是 MeterSphere 导入版，默认补齐模板所需字段：`用例名称`、`所属模块`、`标签`、`前置条件`、`步骤描述`、`预期结果`、`编辑模式`、`备注`、`用例状态`、`责任人`、`用例等级`。
- 对高风险主流程优先生成完整用例；对明显的异常和边界场景也要补充。

3. 输出结果。
- 默认按 [assets/testcase-template.md](assets/testcase-template.md) 输出。
- 默认对齐本地模板 [测试用例模版](<../../wiki/01_MOM产品相关文档/04_测试用例/03_测试用例导入模板/测试用例模版 .xlsx>) 的列结构，不再默认输出旧版“用例编号/用例标题/来源测试点/功能点”表。
- 如用户仅需要文本结果，则默认输出 Markdown 工作稿；工作稿可额外保留 `用例编码` 列，便于评审、引用和修订。
- 如需整理为 MeterSphere 导入版，则移除 `用例编码` 列，并按 Excel 模板同列顺序输出。
...
- MeterSphere 导入版的编辑模式固定填写 TEXT，不得沿用 Markdown 工作稿中的其他值。
...
- `用例编码` 在 Markdown 工作稿中单独保留，例如 `TC-001`、`TC-001A`。
- `用例名称` 默认只写业务标题，不再把 `TC-001` 一类编号前缀直接写进名称本身。
- `标签` 默认取需求名称、模块名称或文档名称；无明确标签时使用当前需求主题名。
- `编辑模式` 默认填写 `TEXT`，`用例状态` 默认填写 `未开始`，`责任人` 无明确信息时留空。
- `备注` 默认承载未单独成列的追踪信息，至少包含：`来源测试点`、`场景类型`、`验证结果`；如果存在不确定规则，再补 `待确认信息`。
- MeterSphere 导入版表格输出时，列顺序必须与模板保持一致；不要擅自增加额外列。
- Markdown 工作稿表格允许在首列增加 `用例编码`，其余字段顺序保持稳定。
...
- 本地 Excel 模版：[测试用例模版](<../../wiki/01_MOM产品相关文档/04_测试用例/03_测试用例导入模板/测试用例模版 .xlsx>)
