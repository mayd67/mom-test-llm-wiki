...
DEFAULT_ENV = "测试环境：http://192.168.30.186:32053/ant-web/#/user/login"
...
def default_output_dir(skill_root: Path) -> Path:
    repo_root = detect_repo_root(skill_root)
    if repo_root is not None:
        return repo_root / "outputs" / "04_测试执行" / "03_缺陷导出"
    return skill_root / OUTPUT_DIRNAME


def resolve_output_path(output_arg: str, today: date, skill_root: Path) -> Path:
    if not output_arg:
        return next_output_path(default_output_dir(skill_root), today)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=str, default="", help="Output CSV file path or directory; in this repository default uses outputs/04_测试执行/03_缺陷导出/jira_import_(MM-DD-NNN).csv")
    parser.add_argument(
        "--template",
        type=str,
